...
void HandleOutput(tflite::ErrorReporter* error_reporter, float x_value,
  ...
  if (!initialized) {
    ...
    BSP_LCD_FillRect(0, 2*BSP_LCD_GetYSize()/8, BSP_LCD_GetXSize(), 4*BSP_LCD_GetYSize()/8);
    initialized = true;
  }
  ...
  // Draw the dot
  BSP_LCD_SetTextColor(LCD_COLOR_GRAY);
  BSP_LCD_FillCircle(x_pos_old, BSP_LCD_GetYSize()/2-height/2+y_pos_old, dot_radius);
